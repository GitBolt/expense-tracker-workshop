...
1. Defining account struct

- 1.1 Define `ExpenseAccount` Struct

2. Defining context

- 2.1 Defining `InitializeExpense` context
- 2.2 Defining `ModifyExpense` context
- 2.3 Defining `DeleteExpense` context

#### 1.1 Define `ExpenseAccount` Struct

Go to [anchor-program/programs/etracker/src/lib.rs](anchor-program/programs/etracker/src/lib.rs)

We will define what values an expense account will store. The space allocated is calculated accordingly.
...
Open up [anchor-program/programs/etracker/src/lib.rs](anchor-program/programs/etracker/src/lib.rs) and checkout `InitializeExpense`  
Notice this part:
...
We are essentially using anchor [space reference](https://book.anchor-lang.com/anchor_references/space.html) to allocate the amount of bytes initializing each new expense will take. We also define the seed of the expense account here. It acts as a hashmap where expenses for specific user are stored against ID's.
We also make sure by using `pub authority: Signer<'info>,` that the address creating the expense account is also signer.

#### 2.2 Defining `ModifyExpense` context

Open up [anchor-program/programs/etracker/src/lib.rs](anchor-program/programs/etracker/src/lib.rs) and checkout `ModifyExpense`  
Notice this part:
...
We are essentially telling anchor that we will be reading and modifying the data in the account defined by the seeds of the expense account here. The seeds are same as `InitializeExpense` as we are modifying the same account initialized in the previous instruction.
We also make sure by using `pub authority: Signer<'info>,` that the address updating the expense account is also signer.

#### 2.3 Defining `DeleteExpense` context

Open up [anchor-program/programs/etracker/src/lib.rs](anchor-program/programs/etracker/src/lib.rs) and checkout `DeleteExpense`  
Notice this part:
...
Since we need to delete the expense we outright close the account which in turn removes all the data stored in it. We also make sure by using `pub authority: Signer<'info>,` that the address deleteing the expense account is also signer.
...
We have to first copy the generated IDL JSON from our program's target directory, and paste it as `IDLType` in our client. This will be the `type`or our IDL Data.
Then, define the `IDL` data variable with the exact same JSON, and add the `IDLType` which we defined as our `IDLData` object's type. That's it!.
